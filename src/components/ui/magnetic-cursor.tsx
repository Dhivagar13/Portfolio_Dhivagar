...
export const MagneticCursor: FC<MagneticCursorProps> = ({
  ...
  useEffect(() => {
    ...
    const activeMagnets = new Map<HTMLElement, boolean>();

    const setupMagneticElement = (el: HTMLElement) => {
      if (activeMagnets.has(el)) return;
      activeMagnets.set(el, true);
      ...
      const handlePointerEnter = () => {
        ...
        const magneticColor = el.getAttribute('data-magnetic-color') || configRef.current.cursorColor;
      ...
      const handlePointerLeave = () => {
        ...
        gsap.to(cursorEl, {
          ...
          backgroundColor: configRef.current.cursorColor,
          scaleX: 1,
          scaleY: 1,
    ...
    const handleGlobalOver = (e: Event) => {
       const target = e.target as HTMLElement;
       const magEl = target.closest<HTMLElement>(`[${hoverAttribute}]`);
       if (magEl) {
           setupMagneticElement(magEl);
       }
    };

    document.addEventListener('pointerover', handleGlobalOver);
    cleanupFunctions.push(() => {
       document.removeEventListener('pointerover', handleGlobalOver);
    });
  ...
  return (
    <>
      <div ref={cursorRef} className={`magnetic-cursor ${cursorClassName}`} style={{...styles, display: 'flex', alignItems: 'center', justifyContent: 'center'}}>
        <span style={{ fontSize: '10px', fontWeight: '900', color: '#000', letterSpacing: '-1px', pointerEvents: 'none' }}>{'<>'}</span>
      </div>
      {children}
    </>
  );
